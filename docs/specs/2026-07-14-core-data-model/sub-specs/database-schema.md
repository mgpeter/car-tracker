...
Rehomed from the deleted importer spec (DEC-008). It survives because it never belonged to the importer:
README §5.3 requires **write paths** to validate mileage monotonicity and flag anomalies rather than accept
them silently, and README §3.2 requires the fuel quick-add to warn on outliers. Both are ordinary application
behaviour. `import_runs` was dropped with the importer.
...
and died with it. What remains are the flags a live write can raise:

| Kind | Trigger |
|---|---|
| `MileageNonMonotonic` | A reading below an earlier-dated one. The schema deliberately permits this (no check constraint); the flag is how §5.3 is honoured. |
...
| `ImplausibleMpg` | An interval MPG outside 10–70 for this vehicle. Usually a missed fill or mistyped odometer, not real economy. Needs the derived-metrics service, so it lands with it. |

**Why `(entity_type, entity_id)` here but three real FKs on `documents`.** README §3.9 names exactly three link
targets for a document, so real FKs get real integrity. An anomaly can attach to any entity, and `entity_id` is
...
`severity` separates "this is wrong" (`Error`) from "expected but worth knowing" (`Info`).
...
even before DEC-008 removed the importer that would have collided with it: seeding a *specific person's car*
...
| Vehicles never seeded | DEC-007; a migration is schema, not one owner's car. Created by the add-car flow or MCP |
| `data_anomalies`, but no `import_runs` | DEC-008; flagging is a write-path obligation (§5.3) that never depended on the importer |
