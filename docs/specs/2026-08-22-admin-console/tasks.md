...
- [x] 1. **The gate** - two permissions, a domain predicate, two policies
  - [x] 1.1 Write tests for `AdminAccess.Grants` in `tests/CarTracker.Domain.Tests/AdminAccessTests.cs`:
        ...
        `admin:read`.
  - [x] 1.2 Add `src/CarTracker.Domain/Admin/AdminAccess.cs` with `PermissionClaim`, `ReadPermission`,
        `PlanWritePermission` and `Grants`.
  - [x] 1.3 Add the `AdminRead` and `AdminPlanWrite` policies to the `AddAuthorization` block at
...
- [x] 2. **The plan override, and extracting the ladder two callers now need**
  - [x] 2.1 Write tests for `PlanResolver.Resolve` in `tests/CarTracker.Domain.Tests/PlanResolverTests.cs`:
        ...
        documented order.
  - [x] 2.2 Add `AccountPlan?  PlanOverride` and `DateTimeOffset? LastSeenAt` to `src/CarTracker.Data/User.cs`
        ...
        **Not anticipated: `AccountPlan` had to move to `CarTracker.Shared` first.** `CarTracker.Data` cannot
        reference `CarTracker.Domain` - the reference runs the other way - so an entity could not hold the
        enum where it was. Moved beside `EntrySource` and `MileageOrigin`, which is the existing precedent for
        an enum an entity stores; `PlanReason`, `PlanResolution` and `PlanAllowances` stayed in the domain
        because nothing stores them. The OpenAPI schema name is unchanged, so the contract did not move.
  - [x] 2.3 Add `PlanReason.AdminGranted` to `src/CarTracker.Domain/Accounts/AccountPlan.cs` with an XML doc
        saying what the reader should do about it, which is nothing.
  - [x] 2.4 Extract `src/CarTracker.Domain/Accounts/PlanResolver.cs` and rewrite
  ...
  - [x] 2.5 Write tests in `tests/CarTracker.Data.Tests/AdminPlanOverrideTests.cs` against real PostgreSQL:
        ...
        the extraction.
  - [x] 2.6 Stamp `LastSeenAt`. **Added during execution: group 2 created the column with no writer, which is
        the writer-less-column trap the spec itself warns about.** `AccountProvisioner.TouchLastSeenAsync`, a
        separate call beside the backfill because `BackfillEmailAsync` returns early at `:225` for every
        established account, plus a stamp at creation because an account created by a sign-in was seen then.
        Tests in `AccountProvisioningTests`: stamped at creation, coalesced at 14 minutes, written at 16, and
        reached on an account whose address needs no repair.
  - [x] 2.7 Verify tests pass.

- [x] 3. **Reading across owners** - the one file allowed to widen the filter
  - [x] 3.1 Write tests for `AdminReadService` in `tests/CarTracker.Data.Tests/AdminReadServiceTests.cs`,
  ...
  - [x] 3.2 Write tests for `RegistrationMask.Mask` in
  ...
  - [x] 3.3 Add `src/CarTracker.Domain/Admin/RegistrationMask.cs`.
  - [x] 3.4 Add `src/CarTracker.Domain/Admin/AdminReadService.cs` and register it scoped in
        ...
        `BypassOwnership`. **Verified by sabotage:** with every `IgnoreQueryFilters()` stripped, four of the
        seven tests in 3.1 go red and three stay green - and they should, because they read `users`,
        `chat_usage` and `vehicle_lookup_usage`, none of which carry a filter. The class comment says exactly
        that rather than claiming all seven prove the widening.
  - [x] 3.5 Verify tests pass.

- [x] 4. **The endpoints**
  - [x] 4.1 Write tests for the pieces that can be tested without a WebApi test project: the diagnostics
  ...
  - [x] 4.2 Add `src/CarTracker.WebApi/Endpoints/AdminEndpoints.cs`: one `MapGroup("/api/admin")` with
        ...
        Register it in the `Program.cs:392-413` block. **Handlers must return the payload or a typed
        `Results<Ok<T>, NotFound>`, not `Results.Ok(...)`:** a bare `IResult` gives OpenAPI no response type,
        so the first cut produced five paths carrying no schema at all and the generated client had nothing to
        import.
  - [x] 4.3 Add `AdminCapabilities` to `AuthenticatedResponse` in `MetaEndpoints.cs`, **required and
  ...
  - [x] 4.4 Log the plan write at Information with the acting `sub`, the target user id and the override
  ...
  - [x] 4.5 Regenerate `api-contract/v1.json` and the TypeScript
...
- [x] 5. **The screen**
  - [x] 5.1 Write tests: `src/screens/AdminPage.test.tsx` (renders the account list; plates in the DOM are
  ...
  - [x] 5.2 Add `useAdminAccess()` to `src/api/queries.ts`, reading the `queryKeys.access` cache entry
        `AuthGate` already fills, and `src/api/admin.ts` with the fetchers and `adminKeys`.
  - [x] 5.3 Wire the shell: `/admin` as a sibling of `:reg` in `routes.tsx` beside `/account`; `CurrentScreen`
  ...
  - [x] 5.4 Build `AdminPage.tsx` and `screens/admin/{DeploymentPanel,AccountsPanel,PosturePanel,AccountSheet}.tsx`.
  ...
  - [x] 5.5 Add a sentence for `AdminGranted` to `REASON` in `screens/account/PlanPanel.tsx:22` - the build
        will already be failing without it, which is that `Record<PlanReason, string>` doing its job.
  - [x] 5.6 Satisfy the guards: add the four `screens/admin/*` panels to `EXEMPT` in `test/coverage.test.ts`
        with the wording the account panels use at `:46-49`; no raw hex (`tokens.test.ts`); any new control
        row declares `flex-wrap: wrap` and `min-width: 0` and is added to `overflow.test.ts` (no new control
        row was needed, so that file is untouched). **Also required and not foreseen: `UserMenu.test.tsx` needs
        a `QueryClientProvider`** - gating the link on a server-supplied capability turned a pure component
        into one with a data dependency.
...
- [x] 6. **Ship it**
  ...
  - [x] 6.3 Document the Auth0 dashboard steps in the README Configuration section: enable **RBAC** *and*
