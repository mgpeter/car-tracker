...
- [ ] 2. Fonts and tokens
  - [ ] 2.1 Decode the three base64 faces from `dashboard-full-claude-design/fonts.css` into `.woff2` under `public/fonts/` (DEC-010)
  - [ ] 2.2 Subset all three faces to Latin and record the size before and after
  - [ ] 2.3 Write `styles/fonts.css` pointing at the extracted `.woff2`: `block` for Oswald (identity, above the fold), `swap` for Inter and JetBrains Mono. Note the design ships `block` for all three — we differ deliberately
  - [ ] 2.4 Port the single semantic token layer into `styles/tokens.css` from `theme.css`, **after diffing it against the forked copies inlined in `dashboard.dc.html` and `fuel-log.dc.html`**; fix `--shadow` missing from `[data-theme="dark"]`, and restore the `/* structure only — never status */` comment on `--accent` that the new theme.css dropped
  - [ ] 2.5 Wire the semantic layer into Tailwind with `@theme inline` and confirm a utility resolves to `var(--bg)` and not a baked hex
  - [ ] 2.6 Write the token test: no component references a raw hex or a palette name (`--ink`/`--paper`/`--green`/`--orange`/`--rust`/`--blue`) — they must use the semantic tokens. **Rewritten 2026-07-15:** the original test ("no file outside tokens.css references a layer-1 palette variable") passed vacuously, because no layer-1 variable exists anywhere outside the field manual
