...
export function checksStatus(checks: VehicleSummary['checks']): ScreenStatus {
  ...
  if (checks.attentionCount > 0) return { tone: 'due', label: `${plural(checks.attentionCount, 'check')} flagged` }
...
export function overallStatus(summary: VehicleSummary): ScreenStatus {
  ...
  if (
    statutory.some((r) => r.urgency === 'Red') ||
    checks.overdueCount > 0 ||
    checks.attentionCount > 0 ||
    mileage.hasNonMonotonicHistory
  ) {
